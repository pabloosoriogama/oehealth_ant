...
class OeHealthLabTests(models.Model):
    ...
    def create(self, vals):
        ...
        cr = self.env.cr
        sql = "select date_analysis,duration from oeh_medical_lab_test WHERE date_analysis >= '"+str(fecha)+" 00:00:00' AND date_analysis <= '"+str(fecha)+" 23:59:59' AND hospital_estudio = '"+str(vals.get('hospital_estudio'))+"';"
        cr.execute(str(sql))
        citas = cr.fetchall()
        if(citas!=[]):
            for c in citas:
...
class OeHealthLabTestsCauses(models.Model):
    ...
    name = fields.Char(string='Código Causes', required=True)
